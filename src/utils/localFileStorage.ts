...
export async function saveAnnotationJSON(
  videoName: string,
  timestamp: number,
  annotationData: any
): Promise<string | null> {
  try {
    ...
    const filename = `${formatTimestamp(timestamp)}_annotation.json`;
    const fileHandle = await videoFolder.getFileHandle(filename, { create: true });
    const writable = await fileHandle.createWritable();
    ...
    console.error('Error saving annotation JSON:', error);
    return null;
  }
}

export async function loadAnnotationJSON(filePath: string): Promise<any | null> {
  try {
    const file = await readFile(filePath);
    if (!file) {
      return null;
...
export async function saveVideoSegmentJSON(
  videoName: string,
  timestamp: number,
  segmentData: any
): Promise<string | null> {
  try {
    ...
    const filename = `${formatTimestamp(timestamp)}_segment.json`;
    const fileHandle = await videoFolder.getFileHandle(filename, { create: true });
    const writable = await fileHandle.createWritable();
    await writable.write(JSON.stringify(segmentData, null, 2));
    await writable.close();

    return `${sanitizeFileName(videoName)}/${filename}`;
  } catch (error) {
    console.error('Error saving segment JSON:', error);
    return null;
  }
}

export async function loadVideoSegmentJSON(filePath: string): Promise<any | null> {
  try {
    const file = await readFile(filePath);
    if (!file) {
      return null;
    ...
    console.error('Error loading segment JSON:', error);
    return null;
  }
}

export async function listVideoFolders(): Promise<string[]> {
  try {
    const rootHandle = await getDirectoryHandle();
    if (!rootHandle) {
      return [];
...
export async function listFilesInFolder(folderName: string): Promise<{ name: string; kind: string }[]> {
  try {
    const rootHandle = await getDirectoryHandle();
    if (!rootHandle) {
      return [];
